...
cons :: f x -> Vec xs f -> Vec (x ': xs) f
cons x (UnsafeVec arr) = runST $ do
  let len = Primitive.sizeofSmallArray arr
  marr <- Primitive.newSmallArray (len + 1) (unsafeToAny x)
  Primitive.copySmallArray marr 1 arr 0 len
  UnsafeVec <$> Primitive.unsafeFreezeSmallArray marr
